...
class HardwareTabMixin:
    def _build_hw_tab(self):
        ...
        hdr.setStyleSheet('font-size:18px; font-weight:bold; color:#7ab8f5;')
        ...
        left_layout.addWidget(mapping_group, 1)
        ...
        right_layout.addWidget(dbc_group, 1)

        # Channel Mapping System Panels
        channel_config_group = QGroupBox('Channel Configuration')
        channel_config_layout = QVBoxLayout(channel_config_group)
        self.channels_view = ChannelsConfigView(self.channel_manager)
        channel_config_layout.addWidget(self.channels_view)
        right_layout.addWidget(channel_config_group, 1)

        channel_mapping_group = QGroupBox('Database to Channel Mapping')
        channel_mapping_layout = QVBoxLayout(channel_mapping_group)
        self.channel_mapping_panel = ChannelMappingPanel(
            self.database_manager, self.channel_manager, self.router
        )
        channel_mapping_layout.addWidget(self.channel_mapping_panel)
        right_layout.addWidget(channel_mapping_group, 1)
        ...
        hil_group = QGroupBox('HIL Integration')
        hil_layout = QVBoxLayout(hil_group)
        self.hil_status_label = QLabel('HIL: Disconnected')
        self.hil_status_label.setStyleSheet('color:#e07070; font-weight:bold;')
        hil_layout.addWidget(self.hil_status_label)

        hil_tool_row = QHBoxLayout()
        hil_tool_row.addWidget(QLabel('Tool:'))
        self.hil_tool_combo = QComboBox()
        self.hil_tool_combo.addItems(['virtual', 'dspace', 'ni'])
        hil_tool_row.addWidget(self.hil_tool_combo)
        hil_tool_row.addStretch()
        hil_layout.addLayout(hil_tool_row)

        self.hil_payload_input = QLineEdit('01 02 03 04')
        self.hil_payload_input.setPlaceholderText('Payload bytes (hex)')
        hil_layout.addWidget(self.hil_payload_input)

        hil_buttons = QHBoxLayout()
        self.hil_connect_btn = QPushButton('Connect HIL')
        self.hil_connect_btn.clicked.connect(self._hil_connect)
        hil_buttons.addWidget(self.hil_connect_btn)
        self.hil_disconnect_btn = QPushButton('Disconnect')
        self.hil_disconnect_btn.clicked.connect(self._hil_disconnect)
        hil_buttons.addWidget(self.hil_disconnect_btn)
        self.hil_send_btn = QPushButton('Send to HIL')
        self.hil_send_btn.clicked.connect(self._hil_send)
        hil_buttons.addWidget(self.hil_send_btn)
        self.hil_receive_btn = QPushButton('Receive')
        self.hil_receive_btn.clicked.connect(self._hil_receive)
        hil_buttons.addWidget(self.hil_receive_btn)
        hil_buttons.addStretch()
        hil_layout.addLayout(hil_buttons)

        self.hil_last_io_label = QLabel('Last HIL IO: —')
        self.hil_last_io_label.setWordWrap(True)
        self.hil_last_io_label.setStyleSheet('color:#b7c6d9;')
        hil_layout.addWidget(self.hil_last_io_label)

        right_layout.addWidget(hil_group, 1)

        splitter.addWidget(right)
        splitter.setSizes([600, 600])
        splitter.setStretchFactor(0, 1)
        splitter.setStretchFactor(1, 1)
        # Give splitter the remaining vertical space so content expands
    ...
    def _update_hil_status(self):
        if not hasattr(self, 'hil'):
            return
        state = self.hil.get_state()
        connected = state.get('connected', False)
        backend = state.get('backend', 'unknown')
        fallback_used = state.get('fallback_used', False)
        last_error = state.get('last_error')
        self.hil_status_label.setText(
            f"HIL: {'Connected' if connected else 'Disconnected'} | Backend: {backend}"
            + (' | Fallback active' if fallback_used else '')
            + (f' | Error: {last_error}' if last_error else '')
        )
        self.hil_status_label.setStyleSheet(
            'color:#44dd44; font-weight:bold;' if connected else 'color:#e07070; font-weight:bold;'
        )
        last_tx = state.get('last_tx')
        last_rx = state.get('last_rx')
        self.hil_last_io_label.setText(
            f"TX count: {state.get('tx_count', 0)} | RX count: {state.get('rx_count', 0)} | "
            f"Last TX: {last_tx.hex().upper() if isinstance(last_tx, (bytes, bytearray)) else last_tx or '—'} | "
            f"Last RX: {last_rx.hex().upper() if isinstance(last_rx, (bytes, bytearray)) else last_rx or '—'}"
        )

    def _hil_connect(self):
        try:
            tool = self.hil_tool_combo.currentText().strip() or 'virtual'
            self.hil.tool = tool
            self.hil.connect()
            self._update_hil_status()
            self.statusBar().showMessage(f'HIL connected via {self.hil.get_state().get("backend", tool)}')
        except Exception as e:
            QMessageBox.warning(self, 'HIL Connection Error', str(e))
            self._update_hil_status()
    ...
    def _hil_disconnect(self):
        try:
            ...
            self._update_hil_status()
            self.statusBar().showMessage('HIL disconnected')

    def _hil_send(self):
        try:
            payload_text = self.hil_payload_input.text().strip()
            ok = self.hil.send_to_hil(payload_text)
            self._update_hil_status()
            if ok:
                self.statusBar().showMessage('Payload sent to HIL')
            else:
                QMessageBox.warning(self, 'HIL Send Failed', self.hil.get_state().get('last_error') or 'HIL send failed')
        except Exception as e:
            QMessageBox.warning(self, 'HIL Send Error', str(e))
            self._update_hil_status()

    def _hil_receive(self):
        try:
            payload = self.hil.receive_from_hil()
            self._update_hil_status()
            if payload is None:
                self.statusBar().showMessage('No HIL data available')
            else:
                self.statusBar().showMessage(f'HIL received: {payload.hex().upper()}')
        except Exception as e:
            QMessageBox.warning(self, 'HIL Receive Error', str(e))
            self._update_hil_status()
