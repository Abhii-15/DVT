...
def _strip_unsupported_dbc_lines(raw_text: str) -> str:
    keep_lines = []
    drop_prefixes = (
...
def _fallback_parse_dbc_text(raw_text: str) -> Tuple[Dict, str]:
    import re
    ...
    msg_count = len(messages)
    sig_count = sum(len(m.get('signals', {})) for m in messages.values())
    summary = f"~{msg_count} messages, ~{sig_count} signals (fallback parser)"
...
def _parse_dbc_file(path: Path) -> Tuple[Dict, str]:
    try:
        ...
        try:
            ...
            sanitized_text = _strip_unsupported_dbc_lines(raw_text)
            try:
                db = cantools.database.load_string(sanitized_text, database_format="dbc", strict=False)
            except Exception:
                return _fallback_parse_dbc_text(raw_text)
...
if _HAS_QT:
    class DBCUploadPanel(QGroupBox):
        ...
        def _build_ui(self) -> None:
            ...
            btn_row = QHBoxLayout()
            self._btn_browse = QPushButton("Load DBC File")
