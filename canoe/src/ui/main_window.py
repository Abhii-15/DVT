...
from PyQt5.QtWidgets import QApplication, QDockWidget, QFileDialog, QLabel, QMainWindow, QMenu, QMessageBox, QPushButton, QSplashScreen, QStatusBar, QTabWidget, QTextEdit, QTreeWidget, QTreeWidgetItem, QVBoxLayout, QWidget
...
class CANoeLikeGUI(QMainWindow, HardwareTabMixin, AnalysisTabMixin, SimulationTabMixin, DiagnosticsTabMixin, TestsTabMixin):
    def __init__(self):
        ...
        self.hil = HILInterface(tool='virtual', allow_fallback=True)
        self.uds = UDSService()
    ...
    def _init_ui(self):
        ...
        # Enable nicer dock behavior and finalize layout after widgets are created
    ...
    def _build_menubar(self):
        ...
        fm.addSeparator()
        fm.addAction('Load DBC…', self._load_dbc)
        fm.addAction('Export Trace CSV', lambda: self._export_trace('csv'))
        fm.addSeparator()
    ...
    def _build_log_dock(self):
        ...
        # Constrain log dock height to avoid large empty areas
        self.log_dock.setMinimumHeight(120)
        self.log_dock.setMaximumHeight(220)
    ...
    def _add_trace_message(self, msg, direction):
        ...
        decoded_signals = decoded if isinstance(decoded, dict) else {}
        decoded_summary = ', '.join(f'{k}={v}' for k, v in decoded_signals.items()) if decoded_signals else ''
        logger.debug(f"Adding trace: ID=0x{msg.arbitration_id:03X}, Name={msg_name}, Direction={direction}, Decoded={decoded_summary}")
        self.trace_model.add_message(msg.timestamp, msg.arbitration_id, msg_name, direction, msg.data, decoded_signals)
        if decoded_signals and hasattr(self, '_record_signal_decodes'):
            self._record_signal_decodes(msg.timestamp, msg.arbitration_id, msg_name, direction, decoded_signals)
    ...
    def _finalize_layout(self):
        try:
            # Ensure docks have sane initial sizes and main window is reasonably sized
            if hasattr(self, 'net_expl_dock'):
                self.addDockWidget(Qt.LeftDockWidgetArea, self.net_expl_dock)
            if hasattr(self, 'log_dock'):
                self.addDockWidget(Qt.BottomDockWidgetArea, self.log_dock)
            if hasattr(self, 'net_expl_dock'):
                self.net_expl_dock.setMaximumWidth(280)
            if hasattr(self, 'log_dock'):
                self.log_dock.setMinimumHeight(120)
                self.log_dock.setMaximumHeight(220)
