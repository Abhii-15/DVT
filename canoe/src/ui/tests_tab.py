...
class TestsTabMixin:
    def _build_test_tab(self):
        ...
        self.load_file_btn = QPushButton('Load Test Folder')
        self.load_file_btn.clicked.connect(self._load_test_folder)
        ...
        self.test_list.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)
        self.test_list.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
        self.test_list.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeToContents)
        self.test_list.horizontalHeader().setSectionResizeMode(3, QHeaderView.Stretch)
        # Make the table expand to fill available vertical space
        self.test_list.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        ...
        rl.addWidget(cl)
        sp.addWidget(right)
        # Prefer more space for the test table and reduce empty gaps
        sp.setSizes([800, 320])
    ...
    def _load_test_file(self):
        # Backwards-compatible single-file loader (not used by default button)
        ...
        try:
            ...
            self._populate_tests_from_list(tests)
    ...
    def _load_test_folder(self):
        folder = QFileDialog.getExistingDirectory(self, 'Load Test Folder', '')
        if not folder:
            return
        try:
            tests = []
            for fn in sorted(os.listdir(folder)):
                ...
                if fn.lower().endswith('.json'):
                    tests.extend(self._parse_json(path))
                elif fn.lower().endswith('.xml'):
                    tests.extend(self._parse_xml(path))
                elif fn.lower().endswith('.txt'):
                    tests.extend(self._parse_txt(path))
            self._populate_tests_from_list(tests)
            self.test_log.append(f'[{time.strftime("%H:%M:%S")}] Loaded {len(tests)} test cases from folder {folder}')
        except Exception as e:
            self.test_log.append(f'[{time.strftime("%H:%M:%S")}] Error loading folder: {e}')

    def _populate_tests_from_list(self, tests):
        self.test_list.setRowCount(len(tests))
