...
from PyQt5.QtWidgets import QSizePolicy, QAbstractItemView, QFileDialog, QGroupBox, QHBoxLayout, QHeaderView, QLabel, QLineEdit, QMessageBox, QPushButton, QSplitter, QTabWidget, QTableWidget, QTableWidgetItem, QTextEdit, QVBoxLayout, QWidget
...
class SimulationTabMixin:
    def _build_simulation_tab(self):
        ...
        canvas_group = QGroupBox('Bus Canvas')
        canvas_layout = QVBoxLayout(canvas_group)
        canvas_layout.setContentsMargins(4, 4, 4, 4)
        canvas_layout.setSpacing(0)
        self.bus_canvas = CANBusGraphicsView()
        # Give the canvas more room and let it expand with the window
        self.bus_canvas.setMinimumHeight(720)
        self.bus_canvas.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        canvas_layout.addWidget(self.bus_canvas)
        vsplit.addWidget(canvas_group)

        bot = QTabWidget()
        bot.setStyleSheet('QTabBar::tab{padding:4px 12px;}')
        
        # Add DBC Upload Panel first (Simulation mode only) - becomes default tab
        self.dbc_upload_panel = DBCUploadPanel()
        # Keep the DBC upload area compact so the canvas remains dominant
        try:
            self.dbc_upload_panel.setMaximumHeight(190)
        except Exception:
            pass
        bot.addTab(self.dbc_upload_panel, 'DBC Files')
        ...
        # Allocate significantly more vertical space to the canvas and less to the bottom tabs
        vsplit.setSizes([860, 180])
