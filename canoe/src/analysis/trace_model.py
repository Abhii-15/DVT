...
class TraceTableModel(QAbstractTableModel):
    ...
    def add_message(self, timestamp, arbitration_id, name, direction, data_bytes, decoded_signals=''):
        ...
        if isinstance(decoded_signals, dict):
            decoded_text = '; '.join(f'{k}={v}' for k, v in decoded_signals.items())
            decoded_payload = json.dumps(decoded_signals, ensure_ascii=False)
        elif isinstance(decoded_signals, (list, tuple)):
            decoded_text = '; '.join(str(item) for item in decoded_signals)
            decoded_payload = decoded_text
        else:
            decoded_text = str(decoded_signals or '')
            decoded_payload = decoded_text
        row = {
            ...
            'Raw Data': raw_data,
            'Decoded Signals': decoded_text,
            'Decoded Signal Data': decoded_payload,
        }
